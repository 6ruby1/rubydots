kitty_keys() {
    ...
    ###
    # repo: https://git.sr.ht/~flexibeast/kitty_keys
    ###

    ###
    # To use it, source this repository's kitty_keys.sh file,
    # e.g. as part of your shell's startup procedure. This will
    # make the kitty_keys function available.
    #
    # When called without arguments, or with any unknown arguments,
    # kitty_keys will print the default keybindings, followed by any
    # custom keybindings specified in the user's kitty configuration file.
    #
    # With a single argument of one of copypaste, debugging, layouts,
    # miscellaneous, scrolling, tabs, windows, print the default keybindings for that category. With a single argument of custom, print only the user's custom keybindings.
    #
    ...
    #
    # Limited configuration over the output format is available
    # via the KITTY_KEYS_LEADING, KITTY_KEYS_TRAILING,
    # KITTY_KEYS_MAX_WIDTH and KITTY_KEYS_CONF environment variables.
    # Details in comments in the source.
    #
    # This function has been lightly tested in the zsh, bash and
    # dash shells. Any non-POSIX shell behaviour is a bug.
    ###
...
"
    ...
    if [ -n "${CHANGES}" ]; then
        ...
        for B in $BINDINGS; do
            if print1 "${DEFAULTS}" | grep -qF "${B}"; then
...
${CUSTOM}"
    ...
    if [ "${WANT_FOOTER}" = 'yes' -a -n "${FOOTER}" ]; then
        print1 "${FOOTER}\n"
    fi
    if [ -n "${TRAILING}" ]; then
        print1 "${TRAILING}" |
